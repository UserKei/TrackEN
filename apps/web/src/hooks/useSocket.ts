...
export const useSocket = () => {
  ...
  const connect = () => {
    ...
    // tree-shaking 热更新的时候保持socket不丢失
    // 生产环境 tree-shaking 是不会生效的 但是热更新的时候会生效
    if (import.meta.hot) {
  ...
  const disconnect = () => {
    if (socket) {
      ...
      if (import.meta.hot) {
        import.meta.hot.data.socket = null; //热更新的时候清空socket
      }
    }
  };
  //获取socket
  const getSocket = (): Socket | null => {
    ...
    if (import.meta.hot && import.meta.hot.data.socket) {
      return import.meta.hot.data.socket; //热更新的时候获取socket
    }
    return null;
  };
