  ...
  <div class="relative w-[800px] h-full bg-linear-to-br from-gray-800 to-gray-900">
    ...
    <div class="absolute top-6 right-6">
      <div class="flex items-center gap-2 bg-white/10 backdrop-blur-sm rounded-lg p-1">
        <button @click="LoadModels('login')" :class="loginClass">
          登录
        </button>
        <button @click="LoadModels('register')" :class="registerClass">
          注册
        </button>
      </div>
    </div>
...
const LoadModels = (url: LoginType) => {
  ...
  emits('changeType', url) // 通知父组件切换了登录注册
}
